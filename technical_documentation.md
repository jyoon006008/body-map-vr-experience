...
2. **Unity Standalone Client (유니티 클라이언트)**: 내장된 HTTP 웹 서버를 통해 웹 클라이언트가 전송한 JSON 데이터를 수신하고, 3D 가상 공간에 실시간으로 실물 크기(1.5배 스케일)의 바디 맵 모델과 플로팅 감정 오브젝트를 렌더링합니다.
...
4. **Connected Component Labeling & Segment Extraction**:
   ...
   * 추출된 영역의 바운딩 박스(`bounding_box`) 좌표와 중심점(`center`) 좌표는 원본 드로잉 크기가 아닌 표준 템플릿 크기(정규화 스페이스)로 캘리브레이션하여 전송합니다.

### C. 데이터 전송 방식 & 브라우저 보안 우회 (HTTPS ➔ HTTP)
* GitHub Pages(`https://...`) 환경에서 로컬 웹 서버(`http://127.0.0.1:8200`)로 요청을 보낼 때 브라우저의 강력한 보안 정책인 **Mixed Content** 차단에 직면하게 됩니다.
* 브라우저 표준 명세상 루프백 주소인 `127.0.0.1`로의 AJAX 요청은 보안 제약에서 제외되므로, 타겟 호스트를 `localhost` 대신 `http://127.0.0.1:8200/upload`로 지정하여 이 제약을 우회하였습니다.
* 네트워크 전송 불가 환경을 대비해 분석 데이터를 로컬 파일로 저장하는 `📥 Download Data (JSON)` 기능도 내장되어 있습니다.

---

## 3. 유니티 VR 클라이언트 기술 사양 (Unity Tech Stack)

유니티 프로젝트는 웹에서 넘어온 바디 맵 정보를 바탕으로 1인칭 가상 공간에 실시간 빌보드 형태의 입체 세그먼트를 구축합니다.

### A. 핵심 개발 환경
* **Unity Engine**: `2022.3.62f3` LTS 버전
* **Render Pipeline**: **URP (Universal Render Pipeline)** 적용
* **Player Controller**: Unity 공식 `StarterAssets` First-Person Controller 패키지 사용 (마우스 룩 및 캐릭터 물리 이동 내장)

### B. 주요 구현 기술 및 C# 스크립트 기능
1. **Asynchronous HTTP Web Server (`BodyMapReceiver.cs`)**:
   * C# `System.Net.HttpListener` 클래스를 사용해 게임 실행과 동시에 포트 `8200`에서 대기하는 백그라운드 웹 서버를 구동합니다.
   * 비동기 스레드 수신 방식을 채택하여 웹에서 대량의 이미지 데이터를 받아 처리하는 순간에도 프레임 드랍(렉)이 전혀 발생하지 않습니다.
2. **Dynamic Mesh & Sprite Mapping (동적 좌표 및 크기 매핑)**:
   * **실물 대비 1.5배 스케일**: 가상 현실에서의 시각적 몰입감을 극대화하기 위해 아바타 실루엣의 물리적 높이를 플레이어 캐릭터 높이(1.8m)의 1.5배인 **2.7m**로 고정 매핑합니다.
   * **동적 PPU 계산**: 넘어오는 텍스트 이미지의 해상도에 맞춰 `pixelsPerUnit = texture.height / 2.7f`를 적용하고, 실루엣 하단이 3D 바닥 평면(`y = -1.8`)에 정확히 접하도록 앵커링합니다.
   * **상대 좌표 보정**: 개별 세그먼트 이미지의 실제 픽셀 가로세로 비와 웹에서 제공한 예상 bounding_box 크기를 비교 계산하여 스케일을 자동 보정해 줍니다.
3. **VR/PC Annotation Edit Mode (실시간 어노테이션 편집 모드)**:
   * `Tab` 키로 편집 모드를 켜고 끌 수 있습니다.
   * **멀티 셀렉트 및 아웃라인**: 레이캐스팅으로 여러 세그먼트를 마우스 클릭으로 선택할 수 있으며, 선택된 영역은 **파란색 아웃라인 쉐이더 효과**로 시각화됩니다.
   * **Merge (합치기, 'M' 키)**: 선택된 2개 이상의 감정 스프라이트를 C# 레벨에서 픽셀 단위로 알파 블렌딩 합성하여 단일 고해상도 Sprite로 병합하고, 색상 정보(예: "Red + Yellow")와 픽셀 면적을 합산 처리합니다.
   * **Delete (삭제, 'Del' 키)**: 오검출되거나 불필요한 감정 세그먼트를 씬에서 제거하고 메모리에서 해제합니다.
...
* **바이너리 제공**: `builds/` 폴더 내에 압축 패키지로 제공됩니다.
  * Windows 빌드: `builds/BodyMapVR_Windows.zip` (추출 시 `BodyMapVR.exe` 실행)
  * macOS 빌드: `builds/BodyMapVR_Mac.zip` (추출 시 `BodyMapVR.app` 번들 실행)
